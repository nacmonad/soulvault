 ...
 *
 * Deploys FRESH SoulVaultSwarm + SoulVaultOrganization to whatever `SOULVAULT_RPC_URL`
...
describe.skipIf(!testnetEnabled)('fund request flow (testnet smoke)', () => {
  ...
  it('deploys fresh contracts and completes one request/approve round-trip', async () => {
    ...
    console.log(`[testnet] Deploying SoulVaultSwarm + SoulVaultOrganization from ${owner.address}...`);
    ...
    // eslint-disable-next-line no-console
    console.log(`[testnet] SoulVaultOrganization @ ${organizationAddress}`);
    ...
    const setOrganizationTx = await swarm.setOrganization(organizationAddress);
    ...
    const organization = new Contract(
      organizationAddress,
      ['function approveFundRequest(address swarm, uint256 requestId)'],
      owner,
    );
    const approveTx = await organization.approveFundRequest(swarmAddress, fundRequestId);
