...
import { buildOrganizationContractProfile, writeOrganizationContractProfile } from '../organization-state.js';
import {
  listFundRequests,
 ...
 * SoulVaultSwarm + SoulVaultOrganization on the user's ens-app-v3 chain. Foundry
 * already covers contract-level edge cases exhaustively (50/50 tests in
 * test/SoulVaultSwarm.t.sol + test/SoulVaultOrganization.t.sol + test/SoulVaultFundRequest.t.sol).
 *
 * This file validates:
 ...
 *   - The merged swarm + organization event watcher with (blockNumber, logIndex) ordering
 ...
 *
 * Setup deploys one Swarm + one Organization (account[0] = owner) and bootstraps
...
describe('fund request flow (CLI integration)', () => {
  ...
  let owner: Wallet; // account[0] — deploys swarm + org, also swarm owner, org owner
  ...
  let swarmAsOwner: Contract; // connected to owner for approveJoin / setOrganization
  let swarmPauseCtl: Contract; // owner-bound with a minimal pause/unpause ABI (not in SOULVAULT_SWARM_ABI)
  let orgRaw: Contract; // connected to owner for raw setup (direct value transfer)
  ...
  beforeAll(async () => {
    ...
    // 1. Deploy fresh SoulVaultSwarm + SoulVaultOrganization.
    ...
    const organizationContract = await deployContract(owner, organizationArtifact);

    swarmAddress = await swarmContract.getAddress();
    organizationAddress = await organizationContract.getAddress();
    ...
    // 2. Register swarm on org, then bind swarm -> organization contract
    const registerSwarmTx = await organizationContract.registerSwarm(swarmAddress);
    await registerSwarmTx.wait();

    const setOrganizationTx = await swarmAsOwner.setOrganization(organizationAddress);
    await setOrganizationTx.wait();
    ...
    const orgProfile = buildOrganizationContractProfile({
      organization: ORG_SLUG,
      contractAddress: organizationAddress,
      ownerAddress: owner.address,
    });
    await writeOrganizationContractProfile(orgProfile);

    // 5. Deposit 10 ether into the org via the CLI helper (validates depositToOrganization).
    await depositToOrganization({
      organization: ORG_SLUG,
      amountEther: '10',
    });
  }, 120_000);
  ...
  it('organization status reports correct balance after deposit', async () => {
    const status = await getOrganizationStatus({ organization: ORG_SLUG });
    expect(status.organization).toBe(ORG_SLUG);
    expect(status.contractAddress.toLowerCase()).toBe(organizationAddress.toLowerCase());
  ...
  it('happy path: request → approve releases funds to the requester', async () => {
    ...
    // the organization owner. Exercises the full ABI wiring and event parsing.
  ...
  it('reject path: does not move funds, status flips to rejected', async () => {
    ...
    // The reject tx is owner-signed and should not touch alice's balance or org balance.
    const requestId = await fileFundRequestAsAlice('2', 'dev tools');
    const orgBefore = await orgRaw.balance();
    const aliceBefore = await provider.getBalance(alice.address);
    ...
    // No funds moved: org balance unchanged, alice's balance unchanged
    // (the reject tx is paid for by the owner, not alice).
    expect(await orgRaw.balance()).toBe(orgBefore);
  ...
  it('pause interaction: swarm paused blocks approve, no state change', async () => {
    ...
    const approved = await approveFundRequestViaOrganization({
      organization: ORG_SLUG,
      swarm: SWARM_SLUG,
      requestId,
    });
  ...
  it('event watcher merges swarm + organization events with correct (block, logIndex) ordering', async () => {
    const batch = await listRecentSwarmEvents({ swarm: SWARM_SLUG });

    expect(batch.organizationAddress?.toLowerCase()).toBe(organizationAddress.toLowerCase());
    expect(batch.events.length).toBeGreaterThan(0);

    // Every event is tagged with its source.
    for (const e of batch.events) {
      expect(['swarm', 'organization']).toContain((e as any).source);
    }

    // We should see both swarm-side fund events AND organization-side events.
    ...
    const organizationEventTypes = new Set(
      batch.events.filter((e: any) => e.source === 'organization').map((e: any) => e.type),
    );

    expect(swarmEventTypes.has('OrganizationSet')).toBe(true);
    ...
    expect(organizationEventTypes.has('FundsDeposited')).toBe(true);
    expect(organizationEventTypes.has('FundsReleased')).toBe(true);
    expect(organizationEventTypes.has('FundRequestRejectedByOrganization')).toBe(true);
    ...
    // (organization) because that's their on-chain order in the same tx.
  ...
  it('organization status reflects spending over the test run', async () => {
