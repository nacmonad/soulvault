...
Implemented in `packages/node/src/clear-sign-modes.ts` and selected per-call:
...
**Speculos side**
- Ledger Nano S+ / Nano X app ELF provisioned under `packages/node/test/speculos/apps/`.
- Fixed 24-word seed committed to `packages/node/test/speculos/seed.txt` (test-only; no production use).
...
Constants are declared in `packages/node/test/speculos/funding.ts` and reused by hardware suites.
...
These typehash strings match `packages/node/src/typed-data.ts` exactly — if you change one, change both.
