...
export async function listRecentSwarmEvents(input: { swarm?: string; fromBlock?: number; toBlock?: number | 'latest' }) {
  ...
  const [
    ...
    contract.queryFilter(contract.filters.TreasurySet(), fromBlock, toBlock),
  ...
  const swarmEvents = [
    ...
    ...treasurySet.map((log) => ({
      ...
      logIndex: (log as any).index ?? 0,
      txHash: log.transactionHash,
      oldTreasury: String((log as any).args?.oldTreasury ?? ''),
      newTreasury: String((log as any).args?.newTreasury ?? ''),
    ...
    ...fundApproved.map((log) => ({
      ...
      requestId: (log as any).args?.requestId?.toString(),
      requester: String((log as any).args?.requester ?? ''),
      treasury: String((log as any).args?.treasury ?? ''),
      amountWei: (log as any).args?.amount?.toString(),
    })),
  ...
  const treasuryAddress = String(await contract.treasury());
  let treasuryEvents: any[] = [];
  if (treasuryAddress !== '0x0000000000000000000000000000000000000000') {
    const provider = contract.runner?.provider;
    if (provider) {
      const treasuryContract = new Contract(treasuryAddress, SOULVAULT_TREASURY_ABI, provider);
      const [deposited, released, rejectedByTreasury, withdrawn] = await Promise.all([
        treasuryContract.queryFilter(treasuryContract.filters.FundsDeposited(), fromBlock, toBlock),
        treasuryContract.queryFilter(treasuryContract.filters.FundsReleased(), fromBlock, toBlock),
        treasuryContract.queryFilter(treasuryContract.filters.FundRequestRejectedByTreasury(), fromBlock, toBlock),
        treasuryContract.queryFilter(treasuryContract.filters.TreasuryWithdrawn(), fromBlock, toBlock),
      ]);
      treasuryEvents = [
        ...deposited.map((log) => ({
        ...
        ...rejectedByTreasury.map((log) => ({
          source: 'treasury' as const,
          type: 'FundRequestRejectedByTreasury',
          blockNumber: log.blockNumber,
          logIndex: (log as any).index ?? 0,
          txHash: log.transactionHash,
          swarm: String((log as any).args?.swarm ?? ''),
  ...
  const events = [...swarmEvents, ...treasuryEvents].sort((a, b) => {
  ...
  return {
    swarm: profile.slug,
    contractAddress: profile.contractAddress,
    treasuryAddress: treasuryAddress !== '0x0000000000000000000000000000000000000000' ? treasuryAddress : undefined,
    fromBlock,
    toBlock,
    events,
  };
...
/** Read the current `treasury()` from the swarm contract. Returns the zero address string if unset. */
export async function readSwarmTreasury(input: { swarm?: string }) {
  const { profile, contract } = await getSwarmContractReadonly(input.swarm);
  const treasuryAddress = String(await contract.treasury());
  return {
    swarm: profile.slug,
    contractAddress: profile.contractAddress,
    treasuryAddress,
    isSet: treasuryAddress !== '0x0000000000000000000000000000000000000000',
  };
}
