...
describe("createLedgerTxChannel — device pricing (legacy, CLI parity)", () => {
  afterEach(() => {
    delete (globalThis as { window?: unknown }).window;
  });

  it("always signs legacy type-0 on the device, even when the RPC estimates fees (CLI parity)", async () => {
    const client = fakeClientWithFees();
    const signTransaction = vi.fn(async (_unsigned: Hex): Promise<DeviceTransactionSignature> => {
      // Legacy conventions: the device returns a single v byte (here the
      // byte-truncated EIP-155 v for Sepolia with yParity = 0).
      return { r: R, s: S, v: 113 };
    });
    const channel = createLedgerTxChannel({
      signTransaction,
    ...
    expect(hash).toBe("0xabc");
    // The fee estimator is never consulted on the device lane.
    ...
    // Broadcast payload carries the reconstructed EIP-155 v.
    const sent = client.request.mock.calls[0][0] as unknown as { params: [Hex] };
  ...
  it("does not broadcast on a user rejection — the operator said no", async () => {
    const client = fakeClientWithFees();
