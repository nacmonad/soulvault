...
- `pnpm test:speculos` runs with blind-sign auto-enabled; `packages/node/src/__integration__/clear-sign-diagnostic.speculos.integration.test.ts` proves the signing pipeline works end-to-end and captures on-device screens.
...
Speculos integration tests (`pnpm test:speculos`) run with blind-signing enabled (auto-toggled by `packages/node/test/global-setup-speculos.ts`). They verify that signing **works** and **captures device screens**, but do not assert field-by-field labels until Ledger CAL serves our descriptors. When the upstream PR lands and rolls out, re-run the suites; the on-device display changes automatically and the existing tests graduate to field-by-field assertions without code changes (the `DISPLAY_CONTRACTS` in `packages/node/src/typed-data.ts` already encode the expected labels).
