...
First, it backs up critical agent markdown state — files like `SOUL.md`, `MEMORY.md`, and `HEARTBEAT.md` — encrypts everything client-side with an epoch key, and stores only ciphertext in 0G Storage.
...
When membership changes, SoulVault rotates the epoch key and publishes wrapped key bundles to offchain encrypted storage so only approved members can decrypt future state.
...
So if a new VPS is needed, a node can request admission, get approved, pull encrypted state, decrypt locally, verify hashes, and boot with full continuity — without exposing plaintext onchain or in remote storage.
