...
1. Clone the app source or download the official ELF:
   ```
   # Nano X example
   curl -L -o packages/node/test/speculos/apps/nanox-ethereum.elf \
     https://github.com/LedgerHQ/app-ethereum/releases/download/<version>/app_nanox.elf
   ```
2. `packages/node/test/speculos/apps/` is gitignored (add entry if not already there).
...
1. **(Recommended)** Install the speculos transport kit and register it with the DMK builder. This repo ships a stub in `packages/node/test/speculos/transport.ts` that dynamically imports `@ledgerhq/device-transport-kit-speculos` if present.
...
> **Checkpoint:** If `pnpm test:speculos` fails with "No Ledger transport registered", add `@ledgerhq/device-transport-kit-speculos` to `packages/node/package.json` and wire it in `packages/node/src/signer.ts` similarly to `nodeHidTransportFactory`. Document the version in `clear-signing-limitations.md`.

## §5 Hardware suite

1. Physical Ledger Nano S+/X, firmware current, Ethereum app installed + current.
2. Device seeded with the **test-only** 24-word seed (`packages/node/test/speculos/seed.txt` line or equivalent). Never use this seed on mainnet.
...
# New Speculos suite
cd packages/node && pnpm test:speculos

# Hardware Ledger suite (operator present)
cd packages/node && pnpm test:ledger
```
...
| `Ledger app ELF not found` | Drop file into `packages/node/test/speculos/apps/nanox-ethereum.elf` or set `SOULVAULT_SPECULOS_APP_ELF`. |
| `ClearSignError code=UNSUPPORTED_SELECTOR` | CAL descriptor missing for the selector in strict mode. Either add a JSON under `packages/node/test/speculos/cal/` or switch that call to `clear-sign-preferred`. |
