...
2. Remove `enableBlindSigning()` from `packages/node/test/global-setup-speculos.ts` (or leave it as fallback for ENS commit only).
3. Upgrade story tests from "signature produced" assertions to "display contract matched" assertions (driven by `DISPLAY_CONTRACTS` in `packages/node/src/typed-data.ts`, which already encodes the expected labels).
