...
**Deploy-time binding:** The constructor takes `address initialTreasury`. Passing a non-zero address binds the swarm to its treasury at deploy time and emits `TreasurySet(address(0), initialTreasury, msg.sender)`. Passing `address(0)` is a first-class supported value meaning "stealth swarm / deferred treasury binding" — the swarm exists without a treasury and can be bound later via `setTreasury`. This lets the CLI reflect the bootstrap order `org create` → `treasury create` → `swarm create` (the swarm is born already bound to the treasury the org published via ENSIP-11 on its ENS name), while still supporting swarms that deliberately operate without a treasury (e.g. agents funded off-band). `setTreasury` remains as a re-binder for migrations or rotation.
