...
Not a PII-Masking-300k leaderboard. SoulVault’s default scan **is** `presidio-web` (pattern recognizers in `packages/presidio-adapter`). OpenAI Privacy Filter is a different product: a 1.5B token classifier (~3 GB weights). We did not download or run it here.
...
Talking-points table for the deck lives in PR 25 / `slides/EthOnline2026/OUTLINE.md`. This note is the evidence we ran *our* engine on a checked-in fixture.
