...
export default async function globalSetup() {
  ...
  const defaultElfPath = path.join(resolveRepoRoot(), `packages/node/test/speculos/apps/${model}-ethereum.elf`);
  const appElf = process.env.SOULVAULT_SPECULOS_APP_ELF ?? defaultElfPath;
  const seedPath = path.join(resolveRepoRoot(), 'packages/node/test/speculos/seed.txt');
