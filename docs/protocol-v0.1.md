...
5. Historical key access for new and recovered joiners (post-MVP)
...
### 4.3 Wrapped Epoch Key Bundle (MVP)
For each approved member in epoch `E`, publish a plaintext JSON bundle containing wrapped key entries.

This plaintext JSON bundle is the intended MVP artifact that gets uploaded to 0G Storage and later fetched for verification via CLI.

MVP schema (plaintext JSON before upload):
```json
{
  "version": 1,
  "swarm": {
  ...
  "entries": {
    "0xRustyBotAddress": {
      "memberName": "RustyBot",
      "wrappedKey": "base64...",
      "pubkeyRef": "agent-pubkey:0xRustyBotAddress"
    },
    "0xAgent2Address": {
      "memberName": "Agent2",
      "wrappedKey": "base64...",
      "pubkeyRef": "agent-pubkey:0xAgent2Address"
    }
  },
...
Equivalent shorthand conceptually:
```json
{
  "RustyBot_epoch_1": "enc(K_epoch, RustyBot_pubkey)",
  "agent2_epoch_1": "enc(K_epoch, agent2_pubkey)"
}
```
...
## 10) Key Loss / VPS Nuke Recovery (post-MVP)
...
### Post-MVP recovery path (Owner Escrow)
Owner escrow and historical recovery remain important, but are explicitly deferred until after MVP while the core swarm lifecycle is stabilized.
...
- Wrapped-key bundles in offchain storage (plaintext JSON of wrapped entries) referenced by `keyBundleRef` + `keyBundleHash`
- `K_epoch` used directly for backup and message encryption in MVP
...
Can defer:
- Owner escrow entry design
- Historical Key Grant CLI command + `grantHistoricalKeys` contract method + `HistoricalKeyBundleGranted` flow
- Key-loss/recovery automation
