...
function wrapContextModuleClearSignAware(inner: ContextModule): ContextModule {
  ...
  // only getContexts is overridden (a hand-rolled object literal breaks
  // typecheck whenever the Ledger SDK adds interface members).
  return new Proxy(inner, {
    get(target, prop) {
      ...
      return async (input: unknown, expectedTypes?: ClearSignContextType[]) => {
        if (!isTransactionSubsetInput(input)) {
          return target.getContexts(input, expectedTypes);
        }
        const mode = resolveMode();
        if (mode === 'blind-only') return [];
        try {
          const ctxs = await target.getContexts(input, expectedTypes);
